...
<details>
  ...
  **문제 :**  
  ...
  권한이 없는 사용자가 URL을 직접 입력해 접근하려고 할 경우를 차단하는 방법에 대해 고민했습니다.  
  단순히 본인만 볼 수 있는 결재문서는 Spring Security의 세션 정보를 활용해 확인할 수 있었지만,  
  부서장, 중간/최종 결재자, 참조자 등 다양한 역할에 따른 접근 권한 부여가 필요했습니다.
  
  **해결 및 개선 :**  
  Access 메서드를 별도로 구현하여 현재 로그인된 사용자의 **역할(Role) 및 PK**를 조회하고,  
  ...
  권한이 없는 사용자가 URL을 직접 입력해 접근할 경우를 효과적으로 차단했습니다.
  
  **장점 :**  
  - **신뢰성 있는 사용자 인증:** SecurityContextHolder를 활용해 현재 사용자의 인증 정보와 권한을 안전하게 조회하고, DB 데이터와 비교하여 무단 접근을 차단함.  
  - **명확한 역할 기반 접근 제어:** 각 역할에 따른 접근 권한을 개별 메서드로 구분하여 구체적이고 세밀한 검증이 가능함.  
  - **URL 직접 입력 방어:** 필요한 권한이 충족되지 않으면 상세보기 페이지에 도달하지 못하도록 설계하여 보안을 강화함.  
  - **모듈화된 승인 검증:** 결재 과정의 각 단계(중간, 최종, 반려)에 대해 독립적인 유효성 검사 메서드를 구현해 체계적인 검증 로직을 관리함.
  
</details>

<details>
  <summary><strong>2. [결재 승인 쿼리 로직] 단계별 서명 노출 최적화 - click</strong></summary>
  ...
  **고민했던 대안 :**  
  1. **서버에서 서명 노출 제어**  
     ...
     - 단점: 불필요한 데이터도 한꺼번에 전송되어 서버 메모리 및 네트워크 부하 증가  
  2. **클라이언트에서 서명 노출 제어**  
     - 서버가 모든 서명 정보를 전달한 후 브라우저에서 처리하는 방식  
     - 단점: 정보 조작으로 인해 보안 취약점 존재  
  3. **DB에서 CASE문 분기 처리 (최종 선택)**  
     - 쿼리 단계에서 결재 상태에 따라 서명 이미지를 조건부로 반환하는 방식  
     - 한 번의 SELECT로 처리되어 서비스 및 프론트 로직이 단순해지고 보안이 강화됨
  
  **해결 및 개선 :**  
  CASE문을 통해 A02(결재중) 상태일 때만 중간 결재자 서명을,  
  A03(결재완료)일 때 최종 결재자 서명을 표시하도록 설계했습니다.  
  이를 통해 승인되지 않은 서명 노출 문제를 해소하고,  
  각 승인 단계별 서명 흐름을 명확하게 분리했습니다.  
  ...
  **장점 :**  
  - **정교한 승인 흐름:** DB에서 한 번에 로직을 분기하여 승인 단계와 서명 이미지 표시 여부를 한눈에 파악할 수 있음.  
  - **보안성 및 유지보수성 강화:** 불필요한 민감 정보 처리를 줄여 보안을 강화하고, 로직 중복을 감소시킴.  
  - **간결한 코드:** 여러 메서드에 분산 구현할 필요 없이 CASE문으로 일괄 처리하여 코드가 간결해짐.
...
<details>
  ...
  **문제 :**  
  테스트 환경에서 약 5,000건 이상의 더미 데이터를 생성해 로딩 속도를 확인한 결과,  
  클라이언트 사이드 렌더링(CSR) 방식으로 모든 데이터를 한꺼번에 로딩하니 성능이 급격히 떨어졌습니다.  
  데이터가 늘어날수록 브라우저의 처리 부담과 대용량 JSON 전송으로 인해 응답 지연이 발생했습니다.
  
  **해결 및 개선 :**  
  Datatables 라이브러리의 AJAX 기능을 활용해 커스텀 페이징 및 검색을 처리하는 방식을 도입했습니다.  
  Mapper에 페이징 및 검색 기능 쿼리를 추가하여 서버에서 필요한 데이터만 전송하도록 개선한 결과,  
  평균 로딩 시간이 3초 이상에서 1초 이하로 단축되었으며, 데이터량 증가에도 안정적인 성능을 유지할 수 있게 되었습니다.  
  ...
  **장점 :**  
  - **페이징 속도 향상:** 클라이언트에서 전체 데이터를 보관·렌더링하는 부담이 줄어 로딩 시간이 크게 단축됨.  
  - **클라이언트 성능 최적화:** 서버에서 미리 정제한 데이터를 전송하여 브라우저 리소스 사용을 최소화함.  
  - **확장성 증가:** 데이터량이 늘어나도 서버에서 직접 페이징 및 검색을 처리해 안정적인 성능을 유지할 수 있음.
...
<details>
  ...
  **문제 :**  
  결재 문서 리스트 구현 시, 각 문서 유형별로 별도의 URL과 매핑 메서드를 사용하여 상세보기 페이지로 이동하도록 구현했으나,  
  반복 코드가 많아 유지보수에 어려움이 있었습니다.  
  - **문제 1:** 새로운 결재 유형 추가 시마다 컨트롤러에 별도의 매핑 추가 필요.  
  - **문제 2:** 각 유형별 메서드에서 결재 첨부파일을 불러오는 코드가 반복되어 가독성과 유지보수성이 저하됨.
  
  **해결 및 개선 :**  
  URL에 `@PathVariable`을 활용하여 결재 문서의 유형과 PK 번호를 포함시켜 단일 `@GetMapping`에서 모든 유형을 처리하도록 리팩토링했습니다.  
  이를 통해 각 문서 유형을 별도로 매핑할 필요 없이 URL 자체로 문서 유형을 식별하고, 유지보수가 용이한 구조로 개선했습니다.  
  ...
  **개선점 :**  
  URL만으로 문서의 상세 내용을 쉽게 파악할 수 있도록 직관적인 URL 구조를 고려할 필요가 있음.  
  현재는 공통 코드(ex: C01, C02)를 사용해 직관성이 다소 떨어짐.
...
<details>
  ...
  **문제 :**  
  결재 문서 작성 시, 각 유형별로 개별 페이지를 만들지, 한 페이지에서 유형 선택에 따라 필요한 항목만 동적으로 표시할지 고민함.
  
  **해결 및 개선 :**  
  유형별 공통 항목이 많음을 고려하여 한 페이지에서 유형 선택 시 필요한 값만 표시하고 나머지는 숨기는 방식을 선택함.  
  숨겨진 항목은 초기화하여 불필요한 데이터 전송을 방지하고, 각 유형별로 입력된 데이터를 분리하여 DB에 저장되도록 설계함.  
  ...
  **장점 :**  
  - 한 페이지에서 모든 유형의 폼을 관리할 수 있어 신규 유형 추가가 용이함.  
  - 공통 필드와 개별 필드의 검증을 분리하여 효율적으로 처리할 수 있음.  
  - 숨겨진 필드의 데이터를 초기화해 불필요한 정보 저장을 방지하고, 필요한 데이터만 서버로 전송하도록 최적화됨.
  
  **개선점 :**  
  유형별 입력 항목이 많아지면 가독성이 떨어지고, 필드 검증이 복잡해질 수 있음.
...
<details>
  ...
  **문제 :**  
  로그인 후 모든 페이지에 표시되는 프로필 이미지가 매번 DB 요청되는 방식은 비효율적임.  
  보안 컨텍스트(Security Context)에 프로필 정보를 저장하는 방식을 도입했으나,  
  프로필 변경 시 컨텍스트에 저장된 정보가 갱신되지 않는 문제가 발생함.
  
  **고민했던 대안 :**  
  - **AJAX를 통한 새로고침 방식:**  
    클라이언트에서 최신 이미지를 비동기적으로 로드하여 동기화 문제를 해결하지만,  
    페이지 이동 시마다 DB 호출이 발생해 서버 부하가 증가함.  
  - **SecurityContextHolder 업데이트 방식 (최종 선택):**  
    프로필 변경 시 보안 컨텍스트에 저장된 사용자 정보를 갱신해 최신 이미지가 반영되도록 함.  
    모든 페이지에 DB 요청 없이 캐시된 정보를 활용하여 서버 부하를 줄임.
  
  **해결 및 개선 :**  
  매 페이지마다 DB에 요청하는 방식 대신 SecurityContextHolder 업데이트 방식을 채택함.  
  - **시큐리티 컨텍스트 동기화:** 프로필 업데이트 시 현재 사용자의 인증 객체를 재생성해 변경된 정보를 즉시 반영하도록 구현함.  
  - **DB와 세션 데이터 일관성 강화:** DB의 최신 프로필 정보를 조회해 인증 객체를 갱신하여 세션과 DB 간 데이터 불일치 문제를 방지함.  
  - **리소스 절약:** AJAX 방식이나 매 페이지 DB 호출 대신 보안 컨텍스트를 활용해 불필요한 DB 호출을 줄이고 서버 리소스 소모를 최소화함.  
  ...
  **장점 :**  
  - **즉각적인 UI 반영:** 프로필 변경 사항이 새로고침 없이 모든 페이지에 즉시 반영됨.  
  - **보안성 유지:** 세션 기반 데이터 동기화 방식으로 안전성을 확보함.  
  - **경량화 솔루션:** 추가 라이브러리 도입 없이 기존 Spring Security로 구현하여 시스템 복잡도를 낮춤.
  
  **개선점 :**  
  - **캐시 최적화:** 브라우저 캐시로 인해 갱신된 이미지가 바로 반영되지 않는 문제를 해결하기 위해 이미지 URL에 타임스탬프 또는 랜덤 파라미터를 추가해 캐시를 무효화하는 방식을 고려해야 함.  
  - **세션 갱신 범위 최적화:** 현재 프로필 변경 시 사용자 전체 정보를 갱신하는 방식으로 동작하여 불필요한 데이터 재생성이 발생함. 향후 필요한 데이터만 갱신하도록 개선할 필요가 있음.
